...
export async function unrestrictLink(url, debrid) {
    try {
...
export async function convertMagnetWithDebrid(magnetUri, debrid) {
    try {
        if (debrid.service === "realdebrid") {
            ...
            // Get torrent info
            const infoResponse = await fetch(`https://api.real-debrid.com/rest/1.0/torrents/info/${addData.id}`, { headers: { Authorization: `Bearer ${debrid.apiKey}` } });
