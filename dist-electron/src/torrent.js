...
export const activeTorrentIds = new Set();
const pendingTorrentWorkerRequests = new Map();
// Callback for events that need mainWindow access
let onTorrentEvent = null;
export function setTorrentEventHandler(handler) {
    onTorrentEvent = handler;
}
export function isTorrentReady() {
    return torrentWorkerReady;
}
export function getStreamServerPort() {
    return streamServerPort;
}
export function callTorrentWorker(message, timeoutMs = 20000) {
...
function handleTorrentWorkerEvent(event, payload) {
...
export async function initTorrentWorker(workerPath) {
    try {
        torrentWorker = new Worker(workerPath);
        torrentWorker.on("message", (msg) => {
            ...
            if (msg.type === "response" && typeof msg.requestId === "string") {
                ...
                if (msg.ok)
                    ...
                    pending.reject(new Error(msg.error || "Torrent worker request failed"));
                return;
            }
            if (msg.type === "event") {
...
export function updateTorrentSeeding(enableSeeding) {
    try {
