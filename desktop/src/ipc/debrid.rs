...
pub fn attach(mut window: FenestraWindow, app: App) -> FenestraWindow {
    ...
    window = register(window, "limbo.isDebridUrlSupported", app.clone(), |app, cmd| {
        let Some(url) = param_str(&cmd.params, "url") else {
            return ok(false);
        };
        ok(app.runtime.block_on(app.debrid.is_url_supported(&app, &url)))
    });
    window = register(window, "limbo.isDebridTorrentSupported", app.clone(), |app, _| {
        ok(app.debrid.is_torrent_supported(&app))
    });
    window = register(window, "limbo.convertMagnetDebrid", app.clone(), |app, cmd| {
    ...
    window = register(window, "limbo.convertTorrentFileDebrid", app.clone(), |app, cmd| {
    ...
    window = register(window, "limbo.getSupportedHosts", app.clone(), |app, _| {
        match app.runtime.block_on(app.debrid.get_supported_hosts(&app)) {
            Ok(hosts) => ok(json!({ "hosts": hosts })),
            Err(error) => ok(json!({ "hosts": [], "error": error.to_string() })),
        }
    });
    window = register(window, "limbo.realDebridDeviceStart", app.clone(), |app, _| {
        match app.runtime.block_on(app.debrid.start_realdebrid_device()) {
            Ok(start) => ok(json!({
                "success": true,
    ...
    window = register(window, "limbo.realDebridDevicePoll", app.clone(), |app, _| {
        app.runtime
            .block_on(app.debrid.poll_realdebrid_device(&app))
            .map_err(|e| BridgeError::new(e.to_string()))
            .and_then(ok)
    });
    window = register(window, "limbo.realDebridDeviceCancel", app.clone(), |app, _| {
