...
import type { MultiPartInfo, Download } from "./types.js";
...
export function parseMultiPartArchive(filename: string): MultiPartInfo {
...
export function areAllPartsCompleted(
  baseName: string,
  downloads: Download[]
): { allCompleted: boolean; part1Path: string | null; totalParts: number; completedParts: number } {
...
export function detectCategory(itemPath: string): string {
  try {
...
export function autoExtractArchiveAsync(
  filePath: string,
  downloadId: string,
  workerPath: string,
...
export function normalizeCliArg(arg: string): string {
  let value = arg.trim();
  if (
...
export function findTorrentFileArg(args: string[]): string | null {
...
export function findMagnetArg(args: string[]): string | null {
  for (const raw of args) {
    const arg = normalizeCliArg(raw);
