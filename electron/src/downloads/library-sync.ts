...
import type { Download, LibraryItem } from "../types.js";

export function addDownloadToLibrary(download: Download): LibraryItem | null {
  try {
...
export function syncLibraryWithFilesystem(): void {
  const library = store.get("library");
  let changed = false;
